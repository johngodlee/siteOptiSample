...
Put pixels and existing plots in the same PCA space. `PCALandscap()` returns two objects with PCA scores, one for pixels (`r_pca`) and one for plots (`p_pca`). `r_pca` is a full PCA object, while `p_pca` is only PCA scores. The PCA is run on variables scaled to unit variance shifted to center on zero.
